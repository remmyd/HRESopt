...
def total_cost_rule(model):
    return (model.total_cost == model.investment_cost + (model.om_cost+model.fuel_cost)*365/model.evaluated_days) # multiplication by 365/evaluated days in case evaluated days is not equal to 365
...
def om_cost_rule(model):
    return (model.om_cost == sum(model.PV_energy_gen_output[t]*model.pv_cost_om/8760
                                +sum(model.WT_energy_gen_output[t,wt]*model.WT_cost_om[wt]/8760 for wt in model.WT)
                                +sum((model.stg_flowOut[t,stg]+model.stg_flowIn[t,stg])*model.stg_cost_om[stg]/8760 for stg in model.Stg)
                                +model.inverter_energy_output[t]*model.inverter_cost_om/8760 
                                +model.rectifier_energy_output[t]*model.inverter_cost_om/8760 
                                +model.GEN_energy_gen_output[t]*model.GEN_cost_om/8760 for t in model.Time)) 
...
def DC_balance_rule(model,t):
    return(model.PV_energy_gen_output[t] 
            + sum((model.stg_flowOut[t,stg] - model.stg_flowIn[t,stg]) for stg in model.Stg)
...
def stg_charge_power_on_rule(model,t,stg): # currently off
    return model.stg_flowIn[t,stg] <= model.stg_a[t,stg]*model.stg_maxCap[stg]

def stg_discharge_power_on_rule(model,t,stg): # currently off
